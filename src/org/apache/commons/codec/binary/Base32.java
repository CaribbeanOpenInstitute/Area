...
public class Base32 extends BaseNCodec {
	...
	/**
	 * BASE32 characters are 5 bits in length. They are formed by taking a block
	 ...
	 * from the "Base32 Alphabet" (as specified in Table 3 of RFC 2045) into
	 * their 5-bit positive integer equivalents. Characters that are not in the
	 * Base32 alphabet but fall within the bounds of the array are translated to
	 * -1.
	 */
	private static final byte[] DECODE_TABLE = {
			...
			-1, -1, 26, 27, 28, 29, 30, 31, -1, -1, -1, -1, -1, -1, -1, -1, // 30-3f
																			// 2-7
	 ...
	 * values into their "Base32 Alphabet" equivalents as specified in Table 3
	 * of RFC 2045.
	 */
	...
	/**
	 * This array is a lookup table that translates Unicode characters drawn
	 * from the "Base32 |Hex Alphabet" (as specified in Table 3 of RFC 2045)
	 * into their 5-bit positive integer equivalents. Characters that are not in
	 * the Base32 Hex alphabet but fall within the bounds of the array are
	 * translated to -1.
	 */
	 ...
	 * 3 of RFC 2045.
	 */
	private static final byte[] HEX_ENCODE_TABLE = { '0', '1', '2', '3', '4',
	 ...
	 * </p>
	 * <p>
	 * Line lengths that aren't multiples of 8 will still essentially end up
	 ...
	 */
	@Override
	void decode(byte[] in, int inPos, int inAvail, Context context) { // package
																		// protected
																		// for
																		// access
																		// from
		...
		if (context.eof && context.modulus >= 2) { // if modulus < 2, nothing to
													// do
			final byte[] buffer = ensureBufferSize(decodeSize, context);
	 ...
	 */
	@Override
	void encode(byte[] in, int inPos, int inAvail, Context context) { // package
																		// protected
																		// for
																		// access
																		// from
		...
		if (inAvail < 0) {
			...
			// CRLF
			if (lineLength > 0 && context.currentLinePos > 0) { // add chunk
																// separator if
																// required
			...
			for (int i = 0; i < inAvail; i++) {
				...
				if (0 == context.modulus) { // we have enough bytes to create
											// our output
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 35)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 30)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 25)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 20)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 15)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 10)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) (context.lbitWorkArea >> 5)
							& MASK_5BITS];
					buffer[context.pos++] = encodeTable[(int) context.lbitWorkArea
							& MASK_5BITS];
