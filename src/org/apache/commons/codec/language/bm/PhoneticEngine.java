 ...
 * <p>
 * This class is intentionally immutable. If you wish to alter the settings for
 * a PhoneticEngine, you must make a new one with the updated settings. This
...
public class PhoneticEngine {
	...
	static final class PhonemeBuilder {
		 ...
		 * languages. This will contain a single phoneme of zero characters.
		 * This can then be appended to. This should be the only way to create a
		...
		/**
		 * Creates a new phoneme builder containing the application of the
		 * expression to all phonemes in this builder.
		 * <p>
		 ...
		 * @return a new phoneme builder containing the results of
		 *         <code>phonemeExpr</code> applied to each phoneme in turn
		 */
		...
		/**
		 * Stringifies the phoneme set. This produces a single string of the
		 * strings of each phoneme, joined with a pipe. This is explicitly
		 * provided in place of toString as it is a potentially expensive
		 * operation, which should be avoided when debugging.
		 * 
	 ...
	 * input sequence at a particular offset. After invocation, the values
	 * <code>i</code> and <code>found</code> are updated. <code>i</code> points
	 * to the index of the next char in <code>input</code> that must be
	 * processed next (the input up to that index having been processed
	 * already), and <code>found</code> indicates if a matching rule was found
	 * or not. In the case where a matching rule was found,
	 * <code>phonemeBuilder</code> is replaced with a new builder containing the
	 * phonemes updated by the matching rule.
	 * 
	 * Although this class is not thread-safe (it has mutable unprotected
	 * fields), it is not shared between threads as it is constructed as needed
	...
	private static final class RulesApplication {
		 ...
		 * one that has a matching context and pattern. Then applies this rule
		 * to the phoneme builder to produce updated phonemes. If there was no
		...
		public RulesApplication invoke() {
			...
			for (Rule rule : this.finalRules) {
				...
				this.phonemeBuilder = this.phonemeBuilder.apply(
						rule.getPhoneme(), maxPhonemes);
				this.found = true;
				break;
			}
	 ...
	 * 
	 * @param cached
	 *            the character sequence to cache
		...
		for (Rule.Phoneme phoneme : phonemeBuilder.getPhonemes()) {
			...
			for (int i = 0; i < phonemeText.length();) {
				RulesApplication rulesApplication = new RulesApplication(
						finalRules, phonemeText, subBuilder, i, maxPhonemes)
						.invoke();
	...
	public String encode(String input, final Languages.LanguageSet languageSet) {
		...
		// rules that apply to a specific language that may be ambiguous or
		// wrong if applied to other languages
		...
		for (int i = 0; i < inputCache.length();) {
			RulesApplication rulesApplication = new RulesApplication(rules,
					inputCache, phonemeBuilder, i, maxPhonemes).invoke();
			i = rulesApplication.getI();
	...
	/**
	 * Gets if multiple phonetic encodings are concatenated or if just the first
	 * one is kept.
	 * 
	 * @return true if multiple phonetic encodings are returned, false if just
	 *         the first is
	 */
