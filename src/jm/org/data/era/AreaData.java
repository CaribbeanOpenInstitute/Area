...
public class AreaData {
	...
	public void updateEsriCategories(){
		Log.e(TAG, "Updating Categories");
		// pull data and put in database
		parser = new JSONParse(context);
		parser.parseESDCategories(dataService.HTTPRequest(0, "http://data.org.jm/esd/categories.json"));
	...
	public void updateEsriIndicators(){
		Log.e(TAG, "Updating Indicators");
		// pull data and put in database
		parser = new JSONParse(context);
		
		parser.parseESDIndicators(dataService.HTTPRequest(0,"http://data.org.jm/esd/indicators.json"));
	...
	*/
	 ...
	 * @return AreaConstants Search Code
	 */
	synchronized public int genericSearch(int dataSource, String indicatorID, String[] country, String category) {
		
		Log.e(TAG, "Generic Search " + indicatorID + " -> " + category);
		...
		if (dataSource == WORLD_SEARCH){
			...
			if(!countries_to_get.isEmpty()){
				// get indicator name and category name for ESD data retrieval
				String ind_str, cat_str;
				Cursor ind_cat;
				ind_cat = rawQuery(INDICATOR, null, "" + WB_INDICATOR_ID + " = '" + indicatorID + "'" );
				ind_cat.moveToFirst();
				ind_str = ind_cat.getString(ind_cat.getColumnIndex(INDICATOR_DESC));
				
				ind_cat = rawQuery(WB_CATEGORY, null, "" + CATEGORY_ID + " = '" + category + "'" );
				ind_cat.moveToFirst();
				cat_str = ind_cat.getString(ind_cat.getColumnIndex(CATEGORY_DESC));
				
				ind_cat.close();
				return getESDCountryIndicators(ind_id, indicatorID, countryIDs, "year=%3E1990", ind_str, cat_str);
	...
	public synchronized int getESDCountryIndicators(int indicator_id, String indicator, ArrayList<Integer> countryIDList, String date,String indicatorStr, String category){
		parser = new JSONParse(context);
		String queryStr = "";
		...
		queryStr = "http://data.org.jm/esd/categories.json?category=" +category+ "&indicator=" + indicatorStr + "&" + date;
		return parser.parseESDData(dataService.HTTPRequest(0,queryStr), indicator_id, countryIDArray, queryStr);
	...
	public synchronized int getBingArticles(String param){
		...
		queryStr = querybase +  "?"+ query + "%27" + paramStr+ "%20Jamaica%20gleaner" + "%27" + "&"  + num_results + "&" + format;
	...
	public String getIndicatorCategory(int indicator) {
		Log.e(TAG, "Get Indicator Name");
		
		String category = "";
		Cursor ind_result, cat_result;

		ind_result = //dbHelper.rawQuery(INDICATOR, null , "" + INDICATOR_ID + " ='" + indicator + "'");
				rawQuery(IND_CATEGORIES, null , "" + I_ID + " ='" + indicator + "'");
		...
		if (ind_result.getCount() != 1){
			...
			ind_result.moveToFirst();
			Log.e(TAG,"" + ind_result.getCount() + " ID: " + ind_result.getString( ind_result.getColumnIndexOrThrow(_ID)));
			cat_result = rawQuery(WB_CATEGORY, null , "" + CATEGORY_ID + " ='"
					+ ind_result.getString(ind_result.getColumnIndexOrThrow(CAT_ID)) + "'");
			cat_result.moveToFirst();
			category = cat_result.getString(cat_result.getColumnIndexOrThrow(CATEGORY_NAME));
		}
		ind_result.close();
		cat_result.close();
		return category;
