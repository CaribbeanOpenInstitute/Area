...
import static jm.org.data.era.AreaConstants.ACTION_BING_UPDATE;
import static jm.org.data.era.AreaConstants.ACTION_FAIL_UPDATE;
import static jm.org.data.era.AreaConstants.ACTION_IDS_UPDATE;
import static jm.org.data.era.AreaConstants.ACTION_WORLD_UPDATE;
import static jm.org.data.era.AreaConstants.BING_SEARCH;
import static jm.org.data.era.AreaConstants.IDS_SEARCH;
import static jm.org.data.era.AreaConstants.RETURN_CNTRY_IDs;
import static jm.org.data.era.AreaConstants.RETURN_COUNTRIES;
import static jm.org.data.era.AreaConstants.RETURN_DATE;
import static jm.org.data.era.AreaConstants.RETURN_IND_ID;
import static jm.org.data.era.AreaConstants.RETURN_KEYWORDS;
import static jm.org.data.era.AreaConstants.RETURN_STRING;
import static jm.org.data.era.AreaConstants.RETURN_WB_IND_ID;
import static jm.org.data.era.AreaConstants.SEARCH_FAIL;
...
/**
 * AreaService Background service used to make API calls to cloud services and
...
public class AreaService extends Service {
	 ...
	 * Search Functions
	 ***********************/
	public int genericSearch(int dSource, String indID, String[] ctry, String category) {
		// Check local cache
		int return_value;
		...
		return_value = area.areaData.genericSearch(dSource, indID, ctry, category);
		...
		if (return_value == SEARCH_SUCCESS) {
			...
			Thread genericSearch = new Thread(new GenericSearchRunnable(
					dSource, return_data));
			genericSearch.start();
		...
		public void run() {
			...
			case WORLD_SEARCH:
				...
				try {
					...
					String indicator = (String) search_data
							.get(RETURN_WB_IND_ID), date = (String) search_data
							.get(RETURN_DATE);
					@SuppressWarnings("unchecked")
					ArrayList<Integer> countryIDList = (ArrayList<Integer>) search_data
							.get(RETURN_CNTRY_IDs);
					@SuppressWarnings("unchecked")
					ArrayList<String> countries = (ArrayList<String>) search_data
							.get(RETURN_COUNTRIES);

					search_id = area.areaData.getCountryIndicators(
							indicator_id, indicator, countries, countryIDList,
							date);
				} catch (Exception e) {
