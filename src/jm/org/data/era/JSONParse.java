...
import static jm.org.data.era.DBConstants.AP_ID;
import static jm.org.data.era.DBConstants.BING_DATE_TIME;
import static jm.org.data.era.DBConstants.BING_QUERY;
import static jm.org.data.era.DBConstants.BING_SEARCH_RESULTS;
import static jm.org.data.era.DBConstants.BING_SEARCH_TABLE;
import static jm.org.data.era.DBConstants.B_S_ID;
import static jm.org.data.era.DBConstants.CAT_ID;
import static jm.org.data.era.DBConstants.COMBINATION;
import static jm.org.data.era.DBConstants.COUNTRY;
import static jm.org.data.era.DBConstants.C_ID;
import static jm.org.data.era.DBConstants.FROM_BING_SEARCH_RESULTS;
import static jm.org.data.era.DBConstants.FROM_CATEGORY;
import static jm.org.data.era.DBConstants.FROM_COUNTRY;
import static jm.org.data.era.DBConstants.FROM_IDS_SEARCH_RESULTS;
import static jm.org.data.era.DBConstants.FROM_INDICATOR;
import static jm.org.data.era.DBConstants.FROM_WB_DATA;
import static jm.org.data.era.DBConstants.IDS_BASE_URL;
import static jm.org.data.era.DBConstants.IDS_DOC_PATH;
import static jm.org.data.era.DBConstants.IDS_OBJECT;
import static jm.org.data.era.DBConstants.IDS_OPERAND;
import static jm.org.data.era.DBConstants.IDS_PARAMETER;
import static jm.org.data.era.DBConstants.IDS_PARAM_VALUE;
import static jm.org.data.era.DBConstants.IDS_P_ID;
import static jm.org.data.era.DBConstants.IDS_SEARCH_PARAMS;
import static jm.org.data.era.DBConstants.IDS_SEARCH_RESULTS;
import static jm.org.data.era.DBConstants.IDS_SEARCH_TABLE;
import static jm.org.data.era.DBConstants.IDS_SITE;
import static jm.org.data.era.DBConstants.IDS_S_ID;
import static jm.org.data.era.DBConstants.IDS_TIMESTAMP;
import static jm.org.data.era.DBConstants.IDS_VIEW_DATE;
import static jm.org.data.era.DBConstants.INDICATOR;
import static jm.org.data.era.DBConstants.IND_CATEGORIES;
import static jm.org.data.era.DBConstants.I_ID;
import static jm.org.data.era.DBConstants.P_ID;
import static jm.org.data.era.DBConstants.QUERY_DATE;
import static jm.org.data.era.DBConstants.QUERY_VIEW_DATE;
import static jm.org.data.era.DBConstants.SC_ID;
import static jm.org.data.era.DBConstants.SEARCH;
import static jm.org.data.era.DBConstants.SEARCH_COUNTRY;
import static jm.org.data.era.DBConstants.SEARCH_CREATED;
import static jm.org.data.era.DBConstants.SEARCH_MODIFIED;
import static jm.org.data.era.DBConstants.SEARCH_URI;
import static jm.org.data.era.DBConstants.SEARCH_VIEWED;
import static jm.org.data.era.DBConstants.S_ID;
import static jm.org.data.era.DBConstants.WB_CATEGORY;
import static jm.org.data.era.DBConstants.WB_COUNTRY_CODE;
import static jm.org.data.era.DBConstants.WB_DATA;
...
public class JSONParse {
	...
	public String parseWB(String jsonData) {
		...
		try {
			...
			jsonText.append("Number of entries "
			...
			for (int i = 0; i < 10; i++) {
				...
				for (int x = 0; x < names.length(); x++) {
					...
					if (jsonInnerObject.optJSONObject(names.getString(x)) == null) {
						jsonText.append("\n" + names.getString(x) + ": "
								+ jsonInnerObject.getString(names.getString(x)));
					} else {
						jsonText.append("\n" + names.getString(x) + ": ");
						jsonText.append("\n\t"
								+ jsonInnerObject.optJSONObject(
										names.getString(x)).toString());
					}
				}
				jsonText.append("\n--------------------------------------\n");
				jsonText.append("--------------------------------------\n\n");
	...
	public int parseBINGData(String jsonData, String params, String uri) {
		...
		try {
			...
			if (numOfrecords > 0) {
				...
				// public static final String[] FROM_BING_SEARCH_TABLE =
			...
			for (int i = 0; i < numOfrecords; i++) {
				...
				for (int a = 0; a < BING_SEARCH_LIST.length; a++) {
					apiRecord.put(FROM_BING_SEARCH_RESULTS[a + 2],
							(String) bing_data.get(BING_SEARCH_LIST[a]));
					// Log.d("Indicators", ""+ FROM_BING_SEARCH_RESULTS[a+2] +
					// ":-> " + (String)bing_data.get(BING_SEARCH_LIST[a]));
				}
	...
	public int parseIDSData(String jsonData, int indicator, int country, String params,
		...
		try {
			...
			int numReturned = Integer.parseInt(metadata
					.getString("total_results"));
			...
			if (numReturned > 0) {
				...
				if (search_id > 0) {
					...
					Log.e(TAG, "Error inserting IDS Search record: Indicator-"
							+ indicator + "URI-" + uri);
					return SEARCH_FAIL;
				}
	...
	public int parseESDData(String jsonData, int indicator_id,
			Integer[] countries, String uri) {
		Log.e(TAG, uri);
		...
		try {
			...
			JSONObject jsonObject = new JSONObject(jsonData);
			JSONArray result = new JSONArray(jsonObject.getString("metadata"));
			
			int numofObjects = result.length();
			...
			if (numofObjects > 0) {
				...
				long date = timeStamp();
				apiRecord = new ContentValues();
				apiRecord.put(I_ID, indicator_id);
				...
				Log.e(TAG,
						"Search_id:"
								+ query_s.getInt(query_s.getColumnIndex(_ID)));
				...
				if (search_id > 0) {
					...
					for (int n = 0; n < countries.length; n++) {
						...
						country_search_id = areaData.insert(SEARCH_COUNTRY,
								apiRecord, 1);
						Log.e(TAG, "" + country_search_id);
					}
				} else {
					Log.e(TAG, "Error inserting Search record: Indicator-"
							+ indicator_id + ", API_ID- 1, " + "URI-" + uri);
					return SEARCH_FAIL;
				}
			...
			for (int i = 0; i < numofObjects; i++) {
				apiRecord = new ContentValues();
				wb_data = parseJSON(wb_data, result.getJSONObject(i), "");
				...
				Cursor SearchCountry = areaData.rawQuery(
						...
						""
								+ C_ID
								+ " = '"
								+ countryData.getInt(countryData
										.getColumnIndex(_ID)) + "' AND " + S_ID
								+ "= '" + search_id + "'");
				SearchCountry.moveToFirst();
				search_country_id = SearchCountry.getInt(SearchCountry
						.getColumnIndex(_ID));
				if (search_country_id < 0) {
					return SEARCH_FAIL;
				}
				apiRecord.put(SC_ID, search_country_id);
				// Log.d("Indicators", ""+ SC_ID + ":-> " +
				// SearchCountry.getInt(SearchCountry.getColumnIndex(_ID)));
				search_country_id = apiRecord.getAsInteger(SC_ID);
				for (int a = 0; a < ESD_DATA_LIST.length; a++) {
					apiRecord.put(FROM_WB_DATA[a + 2],
							(String) wb_data.get(ESD_DATA_LIST[a]));
					Log.d("Indicators", ""+FROM_WB_DATA[a+2] + ":-> " +
							(String)wb_data.get(ESD_DATA_LIST[a]));
				}
				countryData.close();
				SearchCountry.close();
	...
	public int parseWBData(String jsonData, int indicator, Integer[] countries,
			String uri) {
		...
		try {
			...
			for (int i = 0; i < numReturned; i++) {
				...
				for (int a = 0; a < WB_DATA_LIST.length; a++) {
					apiRecord.put(FROM_WB_DATA[a + 2],
							(String) wb_data.get(WB_DATA_LIST[a]));
					// Log.d("Indicators", ""+FROM_WB_DATA[a+2] + ":-> " +
					// (String)wb_data.get(WB_DATA_LIST[a]));
				}
				countryData.close();
	...
	public String parseESDIndicators(String jsonData) {
		...
		try {
			...
			for (int i = 0; i < numReturned; i++) {
				...
				for (int a = 0; a < ESD_IND_LIST.length; a++) {
					apiRecord.put(FROM_INDICATOR[a + 1],
							(String) indicator_data.get(ESD_IND_LIST[a]));
				...
				catRecord.put(CAT_ID, indicator_data.get(ESD_IND_CATEGORY));
	...
	public String parseESDCategories(String jsonData) {
		...
		try {
			...
			for (int i = 0; i < numReturned; i++) {
				...
				category_data = parseJSON(category_data,
						result.getJSONObject(i), "");
				for (int a = 0; a < ESD_CATEGORY_LIST.length; a++) {
					apiRecord.put(FROM_CATEGORY[a + 1],
							(String) category_data.get(ESD_CATEGORY_LIST[a]));
					
					Log.d("Catergories", ""+FROM_CATEGORY[a+1] + ":-> " + (String)category_data.get(ESD_CATEGORY_LIST[a]));
				}
		...
		try {
			...
			for (int i = 0; i < numReturned; i++) {
				...
				for (int a = 0; a < WB_COUNTRY_LIST.length; a++) {
					apiRecord.put(FROM_COUNTRY[a + 1],
